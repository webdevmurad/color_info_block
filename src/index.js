const { colors: COLOR_PALETTE } = require('./colors');
const { icons: ICONS } = require('./icons');
...
class ColoredInfoBlock {
  ...
  normalizeData(data) {
    ...
    newData.level = parseInt(data.level, 10) || this.defaultInfoBlock.number;
  ...
  getTag() {
     ...
     * Add styles class
     */
    WRAP.classList.add(this.CSS.wrapper);
   ...
   */
  get currentLevel() {
    let level = sections.find((levelItem) => levelItem.number === this.DATA.level);
  ...
  selectColor(event) {
    const color = COLOR_PALETTE.find((col) => col.id === Number(event.target.id));
  ...
  selectedActiveIcon(selectedIcon) {
    ...
    if (selectedIcon.id === 'delete') {
      const HEADER_SVG = this.ELEMENT.querySelector('.ce-colored-block-header svg');
      const BODY_SVG = this.ELEMENT.querySelector('.ce-colored-block-body svg');
    ...
    const INPUT_HEADER = this.ELEMENT.querySelector('.ce-colored-block input');

    if (INPUT_HEADER.value) {
      const HEADER_WRAP = this.ELEMENT.querySelector('.ce-colored-block-header');
      const HEADER_SVG = this.ELEMENT.querySelector('.ce-colored-block-header svg');
      const BODY_SVG = this.ELEMENT.querySelector('.ce-colored-block-body svg');
      ...
      const BODY_WRAP = this.ELEMENT.querySelector('.ce-colored-block-body');
      const HEADER_SVG = this.ELEMENT.querySelector('.ce-colored-block-header svg');
      const BODY_SVG = this.ELEMENT.querySelector('.ce-colored-block-body svg');
  ...
  activeColorIcon() {
    const HEADER_SVG = this.ELEMENT.querySelector('.ce-colored-block-header svg path');
    const BODY_SVG = this.ELEMENT.querySelector('.ce-colored-block-body svg path');
  ...
  static getSelectedIconData(idIcon) {
    if (idIcon === 'delete') {
  ...
  static getSelectedColorData(idColor) {
  ...
  get defaultInfoBlock() {
     ...
     */
    if (this.settings.defaultInfoBlock) {
      const userSpecified = sections.find((levelItem) => (
        levelItem.number === this.settings.defaultInfoBlock
      ));
  ...
  static get toolbox() {
    return {
      icon: import('../assets/icon.svg').default,
